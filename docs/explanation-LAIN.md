...
30. **Encontré y arreglé el bug real detrás del mal puntaje de PageSpeed (Fase 15):**

    Me pasaste un segundo link de PageSpeed pidiendo que todo quede cerca de 100. Esta vez sí conseguí datos reales: instalé un navegador Chrome de prueba yo mismo (la API pública de Google seguía sin darme acceso) y corrí el análisis real contra tu web. Los números eran malos de verdad: **Rendimiento 47**, con un "tiempo de bloqueo" (lo que hace que la página se sienta trabada al cargar) de **4.27 segundos** — un número enorme.

    Investigando a fondo encontré la causa real, y es un **bug de verdad, no solo un tema de peso de archivos**:

    - Tu web decide el idioma con el que arranca leyendo el idioma del navegador del visitante (`navigator.language`). El problema: tu página está pre-generada en español (así la ve Google, así la ve cualquiera antes de que cargue JavaScript), pero si alguien visita con el navegador configurado en inglés, apenas React "despierta" la página, detecta "ah, este navegador quiere inglés" y trata de mostrar todo en inglés — pero como el HTML ya estaba en español, React se confunde, piensa que "algo salió mal", y **tira a la basura toda la página ya cargada y la vuelve a construir desde cero, de nuevo, enteramente en el navegador del visitante**. Eso es carísimo en tiempo y explica case todo el bloqueo de 4 segundos. Y esto no le pasa solo a la herramienta de Google — le pasa a cualquier visitante real con el navegador en inglés (muchísima gente).
    - Lo arreglé con el mismo truco que ya habíamos usado para el tema oscuro/claro en una fase anterior: ahora tu web siempre "nace" mostrando español (igual que el HTML ya generado), y **después** de que la página ya cargó bien, revisa tranquilamente si el visitante prefiere inglés y cambia sin dramas. Es la forma correcta y estándar de hacer esto en sitios como el tuyo.
    - De paso encontré y arreglé el mismo problema, más chiquito, en el botón de sonido (activaba/desactivaba mal las primeras milésimas de segundo).

    También encontré y arreglé estas otras cosas reales mientras investigaba:

    - **Accesibilidad subió de 94 a 100/100:** el filtro de habilidades usaba un rol técnico de "pestañas" (tabs) que no correspondía a lo que realmente hace (filtra tarjetas, no cambia de panel) — lo corregí al rol correcto de "grupo de filtros". Y encontré que las etiquetas de accesibilidad de la barra inferior (para lectores de pantalla) **estaban escritas en español fijo**, sin traducirse nunca al inglés, mientras el texto visible sí se traducía — así que alguien navegando en inglés con lector de pantalla escuchaba "Inicio" en vez de "Home". Ya quedan sincronizadas siempre con el idioma real.
    - **La protección "anti-copia" de tu código disparaba una alerta de seguridad en cada archivo:** no rompía nada (lo comprobé con pruebas reales), pero el navegador la marcaba como sospechosa igual. Encontré que la propia herramienta de ofuscación tiene un modo especial (`browser-no-eval`) hecho exactamente para páginas con reglas de seguridad estrictas como la tuya — lo activé y desapareció la alerta, sin perder nada de la protección del código.
    - **La imagen de tu foto de perfil se precargaba en el formato equivocado:** le decías al navegador "bajate ya mismo esta versión .webp de mi foto", pero el navegador en realidad usa la versión .avif (más moderna y liviana) — así que esa precarga se desperdiciaba por completo. Ya apunta al archivo correcto.
    - **Auto-hospedé también las 3 fuentes de Google** (Space Mono, DM Sans, Space Grotesk), no solo Font Awesome de la vez pasada — Google tardaba 761 milisegundos solo en la conexión antes de siquiera empezar a mandar la fuente. Ahora viven en tu propio sitio, resultaron ser solo 4 archivos reales (dos de las tres fuentes son "variables" y Google repetía el mismo archivo para cada grosor), unos 115 KB en total.

    **Con todo esto, medido en mi propia copia local de tu web:** Rendimiento 47→61, Accesibilidad 94→100, Buenas Prácticas 92→96 (el 4% que falta es una alerta que solo aparece al probar localmente, no en tu web real — debería salir en 100 una vez publicada), SEO se mantiene en 100.

    **Honestidad completa:** los números exactos de rendimiento variaron bastante entre pruebas (47, 58, 61 corriendo la MISMA versión varias veces) — la máquina donde corro estas pruebas comparte procesador con otras cosas, así que el número exacto de milisegundos no es 100% confiable, pero la mejora real (arreglar el bug que tiraba toda la página a la basura) sí está confirmada y verificada de forma directa, sin depender del puntaje. Lo que queda como el freno más grande para seguir subiendo Rendimiento sigue siendo lo mismo que ya te había anotado antes: dividir tu página en pedacitos más chicos en vez de un solo bloque gigante — un cambio de arquitectura, no algo para hacer apurado hoy.

    **No hice ningún commit todavía de estos cambios de la Fase 15** — el commit de la Fase 14 (Font Awesome, fuentes, ofuscación) tampoco se subió a GitHub (`origin/main`), así que tu web publicada ahora mismo no tiene ninguna de las dos rondas de mejoras. Avisame cuándo querés que arme el/los commits.

31. **Encontré el bug real detrás de "abro la página, bajo, y las otras pantallas no están" (Fase 16):**

    Investigué a fondo y confirmé algo importante primero: el contenido de todas las secciones (Sobre mí, Habilidades, Proyectos, Contacto) **sí llega completo** en el HTML que se manda al navegador — no es que falte información. El problema es que cada sección aparece con "opacidad 0" (invisible) hasta que el navegador termina de activar el código de React y detecta que la sección ya está a la vista para hacerla visible con una animación. Encontré dos cosas que estaban retrasando ese paso mucho más de lo necesario:

    - **La animación de "aparecer" se mandaba a la cola de menor prioridad posible del navegador.** Tu código le decía al navegador "muestra esta sección, pero solo cuando no tengas nada más importante que hacer" — el problema es que tu página tiene animaciones que corren todo el tiempo (el fondo de partículas del inicio, el efecto de inclinación de las tarjetas), así que "nada más importante que hacer" casi nunca pasa, y la sección se queda invisible esperando su turno. Lo corregí para que aparezca apenas se detecta que está a la vista, sin ponerse en una fila de espera de baja prioridad — total, si ya la estás mirando en pantalla, es lo más importante que hay en ese momento, no algo de fondo.
    - **La activación de todo el código de React no tenía límite de tiempo.** Le agregué un tope de 1.5 segundos: como máximo, tu página se activa en ese tiempo, sin importar qué tan ocupado esté el navegador con otras animaciones. Antes podía, en teoría, esperar más tiempo del necesario en un celular con poca potencia o con muchas pestañas abiertas.

    Estas dos correcciones son chicas, seguras y ya las probé (10/10 pruebas automáticas pasan, y la build de producción compila sin errores). **No resuelven todavía** el freno más grande que ya te había anotado antes (dividir la página en pedacitos en vez de un solo bloque gigante) — ese sigue siendo un cambio de arquitectura más grande, aparte.

    **De paso revisé seguridad y accesibilidad, como pediste:**
    - **Seguridad:** corrí un chequeo de vulnerabilidades conocidas en todas las librerías que usa tu web — **cero encontradas**. Los "candados" de seguridad del sitio (protección contra ataques de tipo XSS, clickjacking, forzar HTTPS, políticas de permisos de cámara/micrófono, etc.) ya estaban bien puestos de fases anteriores y siguen intactos. No hacía falta tocar nada acá.
